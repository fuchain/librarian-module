...
import UIMap from "../configs/ui-mapper.config.js";

export default connect(
  (state, ownProps) => {
    const rawTx = state.Transaction[ownProps.block][ownProps.txId];
    const desc =
      rawTx.operation === "CREATE"
        ? JSON.stringify(extractValue("create.description", rawTx))
        : JSON.stringify(extractValue("transfer.description", rawTx));
