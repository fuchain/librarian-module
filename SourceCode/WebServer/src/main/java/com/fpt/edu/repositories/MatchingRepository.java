...
public interface MatchingRepository extends CrudRepository<Matching, Long> {
	@Query(value = "SELECT m FROM Matching m WHERE m.returnerRequest.id = :returnRequestId" +
		" AND m.status <> :status  AND m.status <> 4")
	...
	@Query(value = "SELECT m FROM Matching m WHERE m.pin = :pin AND m.status <> :status  AND m.status <> 4")
	Matching getByPin(@Param("pin") String pin,
					  @Param("status") int status);

	@Query(value = "SELECT m FROM Matching m WHERE m.book.id = :bookId AND m.status <> :status  AND m.status <> 4")
